...
import { ThreadWorkspacePane, ThreadWorkspaceTabStrip } from "./ThreadWorkspacePane";
import {
...
const EMPTY_THREAD_EDITOR_BUFFERS = Object.freeze({}) as Readonly<Record<string, unknown>>;
...
export default function ChatView(props: ChatViewProps) {
  ...
  const threadWorkspace = useUiStateStore(
    useShallow(
      useCallback(
        (state) => {
          if (!activeThread) {
            return { sourceKey: null, workspace: undefined };
          }

          const scopedWorkspace = state.threadWorkspaceById[routeThreadKey];
          if (scopedWorkspace) {
            return {
              sourceKey: routeThreadKey,
              workspace: scopedWorkspace,
            };
          }

          const legacyWorkspace = state.threadWorkspaceById[activeThread.id];
          return {
            sourceKey: legacyWorkspace ? activeThread.id : routeThreadKey,
  ...
  const setThreadWorkspaceMode = useUiStateStore((state) => state.setThreadWorkspaceMode);
  const activateThreadWorkspaceFile = useUiStateStore((state) => state.activateThreadWorkspaceFile);
  const closeThreadWorkspaceFile = useUiStateStore((state) => state.closeThreadWorkspaceFile);
  const closeOtherThreadWorkspaceFiles = useUiStateStore(
    (state) => state.closeOtherThreadWorkspaceFiles,
  );
  const closeAllThreadWorkspaceFiles = useUiStateStore(
    (state) => state.closeAllThreadWorkspaceFiles,
  );
  ...
  const threadEditorBuffers = useThreadEditorStore(
    useCallback(
      (state) =>
        activeThreadKey
          ? (state.buffersByThreadKey[activeThreadKey] ?? EMPTY_THREAD_EDITOR_BUFFERS)
          : EMPTY_THREAD_EDITOR_BUFFERS,
      [activeThreadKey],
    ),
  );
  const centerPaneMode = threadWorkspace.workspace?.mode === "editor" ? "editor" : "agent";
  const showCenterPaneEditor = centerPaneMode === "editor";
  const showThreadWorkspaceTabs =
    centerPaneMode === "editor" || (threadWorkspace.workspace?.openFilePaths.length ?? 0) > 0;
  const dirtyThreadWorkspaceFiles = useMemo(
    () =>
      new Set(
        (threadWorkspace.workspace?.openFilePaths ?? []).filter((filePath) => {
          const buffer = threadEditorBuffers[filePath] as
            | {
                kind?: string;
          ...
          return (
            buffer?.kind === "text" &&
            buffer.draftContents !== undefined &&
            buffer.savedContents !== undefined &&
            buffer.draftContents !== buffer.savedContents
          );
        }),
      ),
    [threadEditorBuffers, threadWorkspace.workspace?.openFilePaths],
  );
  useEffect(() => {
    if (!activeThread) {
      return;
    }
    if (threadWorkspace.sourceKey !== activeThread.id) {
      return;
    }
    if (routeThreadKey === activeThread.id) {
      return;
    }
    migrateThreadWorkspaceState(activeThread.id, routeThreadKey);
  }, [activeThread, migrateThreadWorkspaceState, routeThreadKey, threadWorkspace.sourceKey]);
  ...
  const onToggleDiff = useCallback(() => {
    ...
    if (!workspaceToolOpen || workspaceToolMode !== "diff") {
      onOpenDiff();
      return;
    }
    void navigate({
  ...
  const onSwitchCenterPaneToAgent = useCallback(() => {
    ...
    setThreadWorkspaceMode(activeThreadKey, "agent");
  }, [activeThreadKey, setThreadWorkspaceMode]);
  const onSwitchCenterPaneToEditor = useCallback(() => {
    if (!activeThreadKey) {
      return;
    }
    setThreadWorkspaceMode(activeThreadKey, "editor");
  }, [activeThreadKey, setThreadWorkspaceMode]);
  const onActivateCenterPaneFile = useCallback(
    (filePath: string) => {
      if (!activeThreadKey) {
        return;
      }
      activateThreadWorkspaceFile(activeThreadKey, filePath);
    },
    [activeThreadKey, activateThreadWorkspaceFile],
  );
  const onCloseCenterPaneFile = useCallback(
    (filePath: string) => {
      if (!activeThreadKey) {
        return;
      }
      closeThreadWorkspaceFile(activeThreadKey, filePath);
    },
    [activeThreadKey, closeThreadWorkspaceFile],
  );
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      >
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          <div className="flex min-h-0 min-w-0 flex-1 flex-col">
            {showThreadWorkspaceTabs ? (
              <ThreadWorkspaceTabStrip
                activeFilePath={threadWorkspace.workspace?.activeFilePath ?? null}
                dirtyFilePaths={dirtyThreadWorkspaceFiles}
                openFilePaths={threadWorkspace.workspace?.openFilePaths ?? []}
                mode={centerPaneMode}
                resolvedTheme={resolvedTheme}
                onOpenAgent={onSwitchCenterPaneToAgent}
                onActivateFile={onActivateCenterPaneFile}
                onCloseFile={onCloseCenterPaneFile}
                onCloseOtherFiles={(filePath) => {
                  if (!activeThreadKey) {
                    return;
                  }
                  closeOtherThreadWorkspaceFiles(activeThreadKey, filePath);
                }}
                onCloseAllFiles={() => {
                  if (!activeThreadKey) {
                    return;
                  }
                  closeAllThreadWorkspaceFiles(activeThreadKey);
                }}
              />
            ) : null}
            {showCenterPaneEditor ? (
              <ThreadWorkspacePane
                activeFilePath={threadWorkspace.workspace?.activeFilePath ?? null}
                cwd={activeWorkspaceRoot ?? null}
                environmentId={activeThread.environmentId}
          ...
          {/* Plan sidebar */}
          {planSidebarOpen && !shouldUsePlanSidebarSheet && !showCenterPaneEditor ? (
            <PlanSidebar
      ...
      {shouldUsePlanSidebarSheet && !showCenterPaneEditor ? (
