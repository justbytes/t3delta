...
import { type ServerProvider, ServerProvider as ServerProviderSchema } from "@t3delta/contracts";
...
export const hydrateCachedProvider = (input: {
  ...
  const hydratedProvider: ServerProvider = {
    ...
    ...((input.cachedProvider.capabilities ?? input.fallbackProvider.capabilities)
      ? { capabilities: input.cachedProvider.capabilities ?? input.fallbackProvider.capabilities }
      : {}),
