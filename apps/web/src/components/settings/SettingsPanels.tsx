...
import { normalizeModelSlug } from "@t3delta/shared/model";
...
import { CURATED_EDITOR_LANGUAGES } from "../../editorLanguageSettings";
...
function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in T3 Delta.",
    };
  }
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="General">
        <SettingsRow
          title="Theme"
          description="Choose how T3 Delta looks across the app."
          resetAction={
            theme !== "system" ? (
        ...
        <SettingsRow
          ...
          resetAction={
            settings.editorCustomAssociations.length > 0 ? (
              <SettingResetButton
                label="custom file associations"
                onClick={() =>
                  updateSettings({
            ...
            <div className="w-full space-y-2">
              {settings.editorCustomAssociations.map((association, index) => (
                <div key={`${association.languageId}-${index}`} className="flex items-center gap-2">
                  <Input
                    className="flex-1"
                    value={association.pattern}
                    onChange={(event) => {
                      const nextAssociations = [...settings.editorCustomAssociations];
                      nextAssociations[index] = {
                        ...association,
                  ...
                  />
                  <Select
                    value={association.languageId}
                    onValueChange={(value) => {
                      const nextLanguageId = value as EditorLanguageId;
                      const nextAssociations = [...settings.editorCustomAssociations];
                      nextAssociations[index] = {
                        ...association,
                  ...
                  >
                    <SelectTrigger
                      className="w-44"
                      aria-label={`Language for association ${index + 1}`}
                    >
                      <SelectValue>
                        {CURATED_EDITOR_LANGUAGES.find(
                          (language) => language.id === association.languageId,
                        )?.label ?? association.languageId}
                      </SelectValue>
                    </SelectTrigger>
                    <SelectPopup align="end" alignItemWithTrigger={false}>
                      {CURATED_EDITOR_LANGUAGES.map((language) => (
                        <SelectItem hideIndicator key={language.id} value={language.id}>
                          {language.label}
                        </SelectItem>
                      ))}
                    </SelectPopup>
                  ...
                  <button
                    type="button"
                    className="inline-flex size-9 items-center justify-center rounded-md border border-border/70 bg-background/70 text-muted-foreground transition-colors hover:border-border hover:bg-background hover:text-foreground"
                    onClick={() =>
                      updateSettings({
                        editorCustomAssociations: settings.editorCustomAssociations.filter(
                          (_, currentIndex) => currentIndex !== index,
                        ),
                      })
                    }
                    aria-label={`Remove file association ${index + 1}`}
                  >
              ...
              <button
                type="button"
                className="inline-flex h-8 items-center gap-1 rounded-md border border-border/70 bg-background/70 px-3 text-xs font-medium text-foreground transition-colors hover:border-border hover:bg-background"
                onClick={() =>
                  updateSettings({
                    editorCustomAssociations: [
