...
function Sidebar({
  ...
  const resolvedResizable = React.useMemo<SidebarResolvedResizableOptions | null>(() => {
    ...
    return {
      ...
      ...(options.onResizeEnd ? { onResizeEnd: options.onResizeEnd } : {}),
      ...
      >
        ...
        <div
          className={cn(
            ...
            containerPosition === "absolute" ? "absolute bottom-0" : "fixed inset-y-0 h-svh",
            side === "left"
...
function SidebarRail({
  allowCollapsedInteractions = false,
  className,
  onClick,
  onPointerCancel,
  onPointerDown,
  ...
  allowCollapsedInteractions?: boolean;
}) {
  ...
  const stopResize = React.useCallback(
    (pointerId: number) => {
      ...
      resolvedResizable?.onResizeEnd?.(resizeState.width);
  ...
  return (
    <button
      ...
      className={cn(
        /* Some callers want the collapsed rail to remain clickable so the panel can be reopened from the edge. */
        "-translate-x-1/2 group-data-[side=left]:-right-4 absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=right]:left-0 sm:flex",
        !allowCollapsedInteractions &&
