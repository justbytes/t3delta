...
import { ServerSettings, ServerSettingsPatch } from "@t3delta/contracts";
import {
  type ClientSettings,
...
import { applyServerSettingsPatch } from "@t3delta/shared/serverSettings";
...
function normalizeClientSettings(settings: Partial<ClientSettings>): ClientSettings {
  return {
    ...DEFAULT_CLIENT_SETTINGS,
    ...settings,
  ...
  const nextHydration = (async () => {
    try {
      ...
      if (persistedSettings) {
        replaceClientSettingsSnapshot(normalizeClientSettings(persistedSettings));
      }
    } catch (error) {
...
function persistClientSettings(settings: ClientSettings): void {
  const normalizedSettings = normalizeClientSettings(settings);
  replaceClientSettingsSnapshot(normalizedSettings);
  void ensureLocalApi()
    .persistence.setClientSettings(normalizedSettings)
    .catch((error) => {
