...
import { isExplicitRelativePath, isWindowsAbsolutePath } from "@t3delta/shared/path";
import {
  insertRankedSearchResult,
  normalizeSearchQuery,
  scoreQueryMatch,
  type RankedSearchResult,
} from "@t3delta/shared/searchRanking";
...
export const makeWorkspaceEntries = Effect.gen(function* () {
  ...
  const listDirectory: WorkspaceEntriesShape["listDirectory"] = Effect.fn(
    ...
    const dirents = yield* Effect.tryPromise({
      try: () => fsPromises.readdir(resolvedDirectory.absolutePath, { withFileTypes: true }),
      catch: (cause) =>
        new WorkspaceEntriesError({
          cwd: input.cwd,
      ...
      .map((dirent): ProjectEntry => {
        const relativePath = toPosixPath(
          resolvedDirectory.relativePath
            ? path.join(resolvedDirectory.relativePath, dirent.name)
            : dirent.name,
        );
        return {
          path: relativePath,
    ...
    return {
      directoryPath: resolvedDirectory.relativePath,
      entries,
    } satisfies ProjectListDirectoryResult;
  });
