import { NetService } from "@t3delta/shared/Net";
...
const EnvServerConfig = Config.all({
  ...
  otlpServiceName: Config.string("T3CODE_OTLP_SERVICE_NAME").pipe(
    Config.withDefault("delta-server"),
  ),
...
const startCommand = Command.make("start", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription("Run the T3 Delta server."),
  Command.withHandler((flags) => runServerCommand(flags)),
);

const serveCommand = Command.make("serve", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription(
    "Run the T3 Delta server without opening a browser and print headless pairing details.",
  ),
  Command.withHandler((flags) =>
...
export const cli = Command.make("delta", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription("Run the T3 Delta server."),
  Command.withHandler((flags) => runServerCommand(flags)),
