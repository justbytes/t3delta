...
import { ServerSettingsError } from "@t3delta/contracts";
...
function resolveCodexModels(
  customModels: ReadonlyArray<string>,
  discoveredModels?: ReadonlyArray<ServerProviderModel>,
): ReadonlyArray<ServerProviderModel> {
  const baseModels =
    discoveredModels && discoveredModels.length > 0 ? discoveredModels : BUILT_IN_MODELS;
...
> {
  ...
  const fallbackModels = resolveCodexModels(codexSettings.customModels);
  ...
  if (!codexSettings.enabled) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "warning",
        auth: { status: "unknown" },
        message: "Codex is disabled in T3 Delta settings.",
      },
    });
  }
  ...
  const discoveredModels = resolveModels
    ? yield* resolveModels({
    ...
    : undefined;
  const models = resolveCodexModels(codexSettings.customModels, discoveredModels);
...
const makePendingCodexProvider = (codexSettings: CodexSettings): ServerProvider => {
  const checkedAt = new Date().toISOString();
  const models = resolveCodexModels(codexSettings.customModels);
