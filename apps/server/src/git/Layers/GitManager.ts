...
} from "@t3delta/shared/git";

import { GitManagerError } from "@t3delta/contracts";
import {
  GitManager,
...
import type { GitManagerServiceError } from "@t3delta/contracts";
import {
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `t3delta/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `t3delta-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
