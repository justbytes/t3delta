...
import { WorkspaceDiagnostics } from "./workspace/Services/WorkspaceDiagnostics.ts";
import { WorkspaceFileSystem } from "./workspace/Services/WorkspaceFileSystem.ts";
import { WorkspaceLanguageServers } from "./workspace/Services/WorkspaceLanguageServers.ts";
...
const makeWsRpcLayer = (currentSessionId: AuthSessionId) =>
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const workspaceDiagnostics = yield* WorkspaceDiagnostics;
      const workspaceFileSystem = yield* WorkspaceFileSystem;
      const workspaceLanguageServers = yield* WorkspaceLanguageServers;
      ...
      return WsRpcGroup.of({
            ...
            workspaceEntries.listDirectory(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Explorer path must stay within the project root."
        ...
        [WS_METHODS.projectsCreateDirectory]: (input) =>
          observeRpcEffect(
            ...
            workspaceFileSystem.createDirectory(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Workspace folder path must stay within the project root."
                  : `Failed to create workspace folder: ${cause.detail}`;
            ...
            workspaceFileSystem.renameEntry(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ...
                  : `Failed to rename workspace entry: ${cause.detail}`;
            ...
            workspaceFileSystem.deleteEntry(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Workspace entry path must stay within the project root."
                  : input.mode === "trash"
                    ? `Failed to move workspace entry to trash: ${cause.detail}`
            ...
            workspaceFileSystem.readFile(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ...
                  : cause.detail.includes("ENOENT")
                    ? "Workspace file no longer exists."
                    : cause.detail.includes("EISDIR")
        ...
        [WS_METHODS.projectsReadDiagnostics]: (input: ProjectReadDiagnosticsInput) =>
          observeRpcEffect(
            WS_METHODS.projectsReadDiagnostics,
            workspaceDiagnostics.readDiagnostics(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Diagnostics path must stay within the project root."
                  : cause.detail.includes("ENOENT")
                    ? "Workspace file no longer exists."
                    : `Failed to read workspace diagnostics: ${cause.detail}`;
                return new ProjectReadDiagnosticsError({
                  message,
                  cause,
                });
              }),
        ...
        [WS_METHODS.projectsLanguageServerStart]: (input: ProjectLanguageServerStartInput) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerStart,
            workspaceLanguageServers.startSession(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server path must stay within the project root."
                  : cause.detail;
                return new ProjectLanguageServerError({
                  message,
                  cause,
                });
              }),
        ...
        [WS_METHODS.projectsLanguageServerStop]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerStop,
            workspaceLanguageServers.stopSession(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server path must stay within the project root."
                  : cause.detail;
        ...
        [WS_METHODS.projectsLanguageServerSyncDocument]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerSyncDocument,
            workspaceLanguageServers.syncDocument(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server document must stay within the project root."
                  : cause.detail;
                return new ProjectLanguageServerError({
                  message,
                  cause,
        ...
        [WS_METHODS.projectsLanguageServerHover]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerHover,
            workspaceLanguageServers.hover(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server path must stay within the project root."
                  : cause.detail;
        ...
        [WS_METHODS.projectsLanguageServerDefinition]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerDefinition,
            workspaceLanguageServers.definition(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server path must stay within the project root."
                  : cause.detail;
        ...
        [WS_METHODS.projectsLanguageServerReferences]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerReferences,
            workspaceLanguageServers.references(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server path must stay within the project root."
                  : cause.detail;
        ...
        [WS_METHODS.projectsLanguageServerCompletion]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerCompletion,
            workspaceLanguageServers.completion(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server path must stay within the project root."
                  : cause.detail;
        ...
        [WS_METHODS.projectsLanguageServerRename]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerRename,
            workspaceLanguageServers.rename(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server path must stay within the project root."
                  : cause.detail;
        ...
        [WS_METHODS.projectsLanguageServerCodeActions]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsLanguageServerCodeActions,
            workspaceLanguageServers.codeActions(input).pipe(
              Effect.mapError((cause) => {
                const message = Schema.is(WorkspacePathOutsideRootError)(cause)
                  ? "Language server path must stay within the project root."
                  : cause.detail;
        ...
        [WS_METHODS.subscribeProjectLanguageServerEvents]: (input) =>
          observeRpcStream(
            WS_METHODS.subscribeProjectLanguageServerEvents,
            workspaceLanguageServers.subscribeEvents(input),
            {
              "rpc.aggregate": "workspace",
            },
          ),
        ...
        [WS_METHODS.gitReadWorkingTreeDiff]: (input) =>
          observeRpcEffect(WS_METHODS.gitReadWorkingTreeDiff, git.readWorkingTreeDiff(input), {
            "rpc.aggregate": "git",
          }),
        ...
        [WS_METHODS.subscribeServerConfig]: (_input) =>
          observeRpcStreamEffect(
            ...
            Effect.gen(function* () {
              ...
              yield* Effect.all(
                [providerRegistry.refresh("hermes"), providerRegistry.refresh("codex")],
                {
