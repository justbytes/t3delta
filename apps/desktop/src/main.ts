...
import type { ContextMenuItem } from "@t3delta/contracts";
import { RotatingFileSink } from "@t3delta/shared/logging";
import { parsePersistedServerObservabilitySettings } from "@t3delta/shared/serverSettings";
...
const APP_USER_MODEL_ID = isDevelopment ? "gg.delta.desktop.dev" : "gg.delta.desktop";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "t3delta-dev.desktop" : "t3delta.desktop";
const LINUX_WM_CLASS = isDevelopment ? "t3delta-dev" : "t3delta";
const USER_DATA_DIR_NAME = isDevelopment ? "t3delta-dev" : "t3delta";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "t3delta (Dev)" : "t3delta (Alpha)";
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { t3deltaCommitHash?: unknown };
    return normalizeCommitHash(parsed.t3deltaCommitHash);
  } catch {
    return null;
  }
}
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("T3 Delta failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `T3 Delta ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/T3 Delta (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`t3delta`). If the legacy
