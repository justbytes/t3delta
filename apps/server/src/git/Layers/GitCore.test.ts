...
import { GitCommandError } from "@t3delta/contracts";
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "t3delta/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "t3delta/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "t3delta/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "t3delta/tmp-working",
          newBranch: "t3delta/feat/session",
        });

        expect(renamed.branch).toBe("t3delta/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "t3delta/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "t3delta/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("t3delta/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "t3delta/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "t3delta/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "t3delta/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "t3delta/tmp-working" });
        ...
        expect(renamed.branch).toBe("t3delta/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:pingdotgg/t3delta.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:pingdotgg/t3delta.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "t3delta/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
