...
function listUiThreadSyncInputs() {
  const threads = selectThreadsAcrossEnvironments(useStore.getState());
  const serverThreadInputs = threads.map((thread) => ({
    key: scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)),
    seedVisitedAt: thread.updatedAt ?? thread.createdAt,
  }));
  const serverThreadKeySet = new Set(serverThreadInputs.map((thread) => thread.key));
  const draftThreadInputs = useComposerDraftStore
    .getState()
    .listDraftThreadKeys()
    .flatMap((key) => (serverThreadKeySet.has(key) ? [] : [{ key }]));

  return {
    threads,
    uiThreadInputs: [...serverThreadInputs, ...draftThreadInputs],
  };
}

function syncThreadUiFromStore() {
  const { threads, uiThreadInputs } = listUiThreadSyncInputs();
  useUiStateStore.getState().syncThreads(uiThreadInputs);
  markPromotedDraftThreadsByRef(
...
function applyRecoveredEventBatch(
  ...
  if (needsThreadUiSync) {
    const { uiThreadInputs } = listUiThreadSyncInputs();
    useUiStateStore.getState().syncThreads(uiThreadInputs);
  }
