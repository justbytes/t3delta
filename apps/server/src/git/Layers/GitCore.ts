...
import { GitCommandError, type GitBranch } from "@t3delta/contracts";
import { dedupeRemoteBranchesWithLocalMatches } from "@t3delta/shared/git";
...
import { decodeJsonResult } from "@t3delta/shared/schemaJson";
...
const WORKING_TREE_DIFF_PATCH_MAX_OUTPUT_BYTES = 59_000;
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `t3delta-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
...
export const makeGitCore = Effect.fn("makeGitCore")(function* (options?: {
  ...
  const readWorkingTreeDiff: GitCoreShape["readWorkingTreeDiff"] = Effect.fn("readWorkingTreeDiff")(
    function* (input) {
      const headRef = yield* runGitStdout(
        "GitCore.readWorkingTreeDiff.headRef",
        input.cwd,
      ...
      const diff = yield* runGitStdoutWithOptions(
        ...
        headRef.length > 0
          ...
          : ["diff", "--cached", "--patch", "--minimal"],
        {
          maxOutputBytes: WORKING_TREE_DIFF_PATCH_MAX_OUTPUT_BYTES,
          truncateOutputAtMaxBytes: true,
        },
      );
