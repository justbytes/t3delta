...
import { EnvironmentId, ProjectId, ThreadId, TurnId } from "@t3delta/contracts";
...
const makeThread = (input?: {
  ...
  title: "Thread",
  modelSelection: { provider: "hermes" as const, model: "gpt-5.4" },
...
describe("hasServerAcknowledgedLocalDispatch", () => {
  ...
  it("does not clear local dispatch before server state changes", () => {
    const localDispatch = createLocalDispatchSnapshot({
      ...
      projectId,
      title: "Thread",
      modelSelection: { provider: "hermes", model: "gpt-5.4" },
