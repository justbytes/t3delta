...
const workspaceAndProjectServicesLayer = Layer.mergeAll(
  ...
  Layer.mock(WorkspaceDiagnostics)({
    readDiagnostics: () => Effect.succeed({ relativePath: "", diagnostics: [], runs: [] }),
  }),
  Layer.mock(WorkspaceLanguageServers)({
    startSession: (input) =>
      Effect.succeed({
        session: { cwd: input.cwd, serverId: input.serverId },
    ...
    stopSession: (input) =>
      Effect.succeed({ session: { cwd: input.cwd, serverId: input.serverId }, stopped: true }),
    syncDocument: () => Effect.void,
    hover: () => Effect.succeed({}),
    definition: () => Effect.succeed({ locations: [] }),
    references: () => Effect.succeed({ locations: [] }),
    completion: () => Effect.succeed({ items: [] }),
    rename: () => Effect.succeed({ edits: [] }),
    codeActions: () => Effect.succeed({ actions: [] }),
  ...
  it.effect("enriches replayed project events with repository identity metadata", () =>
    Effect.gen(function* () {
      const repositoryIdentity = {
        canonicalKey: "github.com/t3tools/t3delta",
        locator: {
          source: "git-remote" as const,
          remoteName: "origin",
          remoteUrl: "git@github.com:T3Tools/t3delta.git",
        },
        displayName: "T3Tools/t3delta",
        provider: "github",
        owner: "T3Tools",
        name: "t3delta",
      };
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasOriginRemote: true,
            isDefaultBranch: false,
            branch: "t3delta/bootstrap-branch",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          cwd: "/tmp/project",
          branch: "main",
          newBranch: "t3delta/bootstrap-branch",
          path: null,
        });
