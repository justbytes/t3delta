...
import { deepMerge } from "@t3delta/shared/Struct";
...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest()))(
  ...
  (it) => {
    ...
    describe("checkCodexProviderStatus", () => {
      ...
      it.effect("prefers the live codex model list when discovery returns newer models", () =>
        Effect.gen(function* () {
          yield* withTempCodexHome();
          const status = yield* checkCodexProviderStatus(
            () =>
      ...
      it.effect("returns unavailable when codex is below the minimum supported version", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            status.message,
            "Codex CLI v0.36.0 is too old for T3 Delta. Upgrade to v0.37.0 or newer and restart T3 Delta.",
          );
        }).pipe(
          Effect.provide(
            mockSpawnerLayer((args) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("does not probe provider health during registry startup", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.deepStrictEqual(
              (yield* registry.getProviders).map((provider) => provider.provider),
              ["hermes", "codex"],
            );
      ...
      it.effect("reruns codex health when codex provider settings change", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.deepStrictEqual(
              initial.map((provider) => provider.provider),
              ["hermes", "codex"],
            );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in T3 Delta settings.");
        }),
      );
    });
